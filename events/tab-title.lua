...
M.colors = {
    default = {bg = "#589220", fg = "#1c1b19"},
    is_active = {bg = "#dac835", fg = "#11111b"},

    hover = {bg = "#79c92e", fg = "#1c1b19"}
}
...
M.check_if_admin = function(p)
    if p:match("^Administrator: ") then return true end
    return false
end
...
M.setup = function()
    wezterm.on("format-tab-title",
        ...
        local bg
        local fg
        local process_name = M.set_process_name(tab.active_pane
                                                    .foreground_process_name)
        local is_admin = M.check_if_admin(tab.active_pane.title)
        local title = M.set_title(process_name, tab.tab_title,
                                  tab.active_pane.title, max_width,
                                  (is_admin and 8))
